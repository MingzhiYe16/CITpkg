...
// [[Rcpp::export]]
void citbinmp(Rcpp::NumericVector L, Rcpp::NumericVector G, Rcpp::NumericVector T,
			  int &maxit, int &permit, int &permnum, int &nobs, int &dfz, int &dfx,
			  Rcpp::NumericVector pval1, Rcpp::NumericVector pval2, Rcpp::NumericVector pval4, Rcpp::NumericVector pval3nc, Rcpp::IntegerVector perm_index, int &rseed)
{
	...
	designmat = new double[nobs * (dfz + dfx + 1)];
	...
	for (perm = 0; perm < (permnum + 1); perm++)
	{
		...
		stride = 1 + dfz; // intercept + covariates + multiple L variable
		rind = 0;
		for (rw = 0; rw < nobs; rw++)
		{
			brw = permind[rw];
			...
			if (aa)
			{
				...
				for (cl = 0; cl < dfz; cl++)
				{
					designmat[rw * stride + 1 + cl] = LL[brw][cl];
				}
				rind++;
			} // end if aa
		} // end for rw
		...
		df = dfz;
		converged = logisticReg(pv, phenovec, designmat, rind, stride, df);
		if (!converged)
		...
		for (rw = 0; rw < nobs; rw++)
		{
			...
			if (aa)
			{
				...
				for (cl = 0; cl < dfz; cl++)
				{
					designmat[rw * stride + 1 + cl] = LL[rw][cl];
				}
				for (cl = 0; cl < dfx; cl++)
				{
					designmat[rw * stride + 1 + dfz + cl] = XX[brw][cl];
				}
				rind++;
			} // end if aa
		} // end for rw
		...
		for (rw = 0; rw < nobs; rw++)
		{
			...
			if (aa)
			{
				...
				for (cl = 0; cl < dfx; cl++)
				{
					designmat[rw * stride + 1 + cl] = XX[rw][cl];
				}
				for (cl = 0; cl < dfz; cl++)
				{
					designmat[rw * stride + 1 + dfx + cl] = LL[brw][cl];
				}
				rind++;
			} // end if aa
		} // end for rw
		...
		if (perm > 0)
		{
			...
			for (rw = 0; rw < nobs; rw++)
			{
				...
				if (aa)
				{
					...
					for (cl = 0; cl < dfx; cl++)
					{
						designmat[rind * stride + 1 + cl] = Gp[rw][cl];
					}
					for (cl = 0; cl < dfz; cl++)
					{
						designmat[rind * stride + 1 + dfx + cl] = LL[rw][cl];
					}
					rind++;
				} // end if aa
			} // end for rw
			...
			df = dfz;
			converged = logisticReg(pvp, phenovec, designmat, rind, stride, df);
			if (!converged)
				Rcpp::Rcout << "Warning: Cannot Converge when doing regression for calculating P-value." << std::endl;
	...
	for (perm = 0; perm < (permnum + 1); perm++)
	{
		pval4[perm] = (perm == 0) ? 1.0 * npos / nperm : 1.0 * nposperm[perm - 1] / nperm; // pval for L ind T|G
	} // End perm loop
